...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if RANK in {-1, 0}:
        ...
        # (+) -> add by billy: qat model save to onnx
        quant_model = quant_model.cpu().eval()
        quant_model = torch.quantization.convert(quant_model, inplace=True)
        torch.save(quant_model.state_dict(), w / "qat.pt")  
        # quant_model.load.state_dict(torch.load( w / "qat.pt")).eval()
        from models.yolo import Detect
        from models.common import Conv
        for m in quant_model.modules():
            t = type(m)
            if t in (Detect, Model):
                m.inplace = False
                if t is Detect:
